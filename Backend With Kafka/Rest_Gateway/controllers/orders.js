...
exports.addOrderDetails = asyncHandler(async (req, res, next) => {
  ...
  const orderSummaryPayload = {
    custName: cart.custName,
    custid: cart.custid,
    modeOfDelivery: cart.modeOfDelivery,
    ...
    uniqueItems: req.body.uniqueItems,
    orderStatus: "Order Received",
    deliveryStatus: "New Order",
    dishes: req.body.items,
  };
  const msg = { orderSummaryPayload, role: req.user.role};
  kafka.make_request("post_addOrderDetails", msg, function (error, orderData) {
    if (error) {
...
exports.getOrderDetails = asyncHandler(async (req, res, next) => {
    ...
    kafka.make_request("get_getOrderDetails", req.params, function (error, orderData) {


        if (error) {
            if(error.msg==="No order found"){
...
exports.getAllOrdersCustomer = asyncHandler(async (req, res, next) => {
 ...
 const msg={reqQuery:{ ...req.query }, id:req.user.id}

    kafka.make_request("get_getAllOrdersCustomer", msg, function (error, orderData) {
        ...
        if(error){
            ...
            if(error.msg==="Not owner"){
                return next(new ErrorResponse(`Not aurhorised`, 401));
            }
            res.status(500).json({
                success: false,
                msg: "Internal server error"
...
exports.getAllOrdersRestaurant = asyncHandler(async (req, res, next) => {
  ...
  const msg={reqQuery:{ ...req.query }, id:req.user.id}
     ...
     kafka.make_request("get_getAllOrdersRestaurant", msg, function (error, orderData) {
...
exports.filterOrderDetailsCust = asyncHandler(async (req, res, next) => {
  ...
  const msg={reqQuery:{...req.query}, body:req.body, id:req.user.id}
 
     kafka.make_request("post_filterOrderDetailsCust", msg, function (error, orderData) {
...
exports.filterOrderDetailsRest = asyncHandler(async (req, res, next) => {
  ...
  const msg={reqQuery:{...req.query}, body:req.body, id:req.user.id}
     ...
     kafka.make_request("post_filterOrderDetailsRestaurant", msg, function (error, orderData) {
...
exports.updateOrderStatus = asyncHandler(async (req, res, next) => {
    ...
    kafka.make_request("update_updateOrderDetails", req.body, function (error, orderData) {
...
//   if (!order) {
//     return next(
//       new ErrorResponse(`Order not found with id ${req.body.orderId}`, 404)
//     );
//   }
...
exports.getDish = asyncHandler(async (req, res, next) => {
  ...
  if (!dish) {
    return next(
      new ErrorResponse(`No dish found with id ${req.params.id}`, 400)
    );
  }
